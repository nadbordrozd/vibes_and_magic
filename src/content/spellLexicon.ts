...
export const SPELL_LEXICON: Record<SpellLexiconId, SpellLexiconEntry> = {
  ...
  barrowfield: entry('barrowfield', 'Barrowfield',
    ...
    ['barrowfield', 'barrowfields'], ['barrowfield', 'barrowfields', 'barrow', 'barrows']),
  ...
  'company-status': entry('company-status', 'Company status',
    ...
    ['company status', 'company statuses', 'status effect', 'status effects'],
  ...
  deepwood: entry('deepwood', 'Deepwood',
    'A Wild-resonant forest terrain. Greenway travels through connected Deepwood; Green Tide changes movement through all Deepwood.',
    'three close old trees whose roots form a clear path opening',
    ['deepwood', 'deepwoods', 'connected deepwood'], ['deepwood', 'deepwoods', 'forest', 'forests']),
  'extra-action': entry('extra-action', 'Extra action',
    'An additional company action. Reaching the Morale threshold can grant one; it does not give the hero another spell cast unless another rule says so.',
  ...
  'spell-power': entry('spell-power', 'Spell Power',
    ...
    ['spell power', 'spell-power'], ['spell', 'power', 'scaling', 'duration', 'magnitude', 'percentage']),
  summon: entry('summon', 'Summon',
    'Create a company or object. A summoned battle company does not join the hero’s permanent army after battle.',
  ...
  undergrowth: entry('undergrowth', 'Undergrowth',
    'On a battlefield, undergrowth slows movement and an Upgraded thicket gives Chill to enemies that end there. On the adventure map, created undergrowth is impassable.',
...
export const SPELL_MECHANICS_COVERAGE: Record<SpellId, SpellMechanicsCoverage> = {
  ...
  standingMirror: coverage('combat:expansion.standingMirror', ['summon', 'spell-power', 'twister'], ['actors', 'targets', 'learning']),
