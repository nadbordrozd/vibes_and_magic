...
export function adventurePath(
  ...
  return findMixedPath(
    state.map, hero.position, destination, blockedMapTiles(state, hero, options), hero, state.omen,
    freeForest,
  );
}
...
function findMixedPath(
  ...
  const open = new Map([[key(initial), initial]]);
  const distance = new Map([[key(initial), 0]]);
  const previous = new Map<string, string>();
  const nodes = new Map([[key(initial), initial]]);
  while (open.size) {
    const currentKey = [...open.keys()].sort((a, b) =>
      (distance.get(a) ?? Infinity) - (distance.get(b) ?? Infinity) || a.localeCompare(b))[0];
    ...
    for (const candidateNode of candidates) {
      ...
      nodes.set(nextKey, next); open.set(nextKey, next);
    }
  }
  return null;
}
