...
|---|---|---|---|---|---|
| RB-001 | High | `vibes_and_magic-cyi.1` | Gauntlet of the Second Throw adds one hex to the first Gale or push each battle. | Artifact has `push_bonus` in [`artifacts.ts`](../../src/content/artifacts.ts), but no non-catalog consumer or per-battle spent state exists. | Add a generic first-push modifier and serialized/spent battle-side state; cover spell and unit forced movement. |
| RB-002 | High | `vibes_and_magic-cyi.2` | Candle-Snuffer’s Ring reduces enemy death-trigger magnitude by one. | Artifact has `reduce_enemy_death`, but the death-trigger pipeline never queries it. | Define magnitude-floor semantics for counter/morale triggers without weakening percent-of-self triggers, implement at `death-triggers`, and test both sides. |
| RB-003 | High | `vibes_and_magic-cyi.3` | The Long Spoon provides a once-per-battle action that consumes any counter pile and grants `5×N` morale. | Artifact has `eat_counter`, but no combat action, legal target, or usage state exists. | Add an explicit replayable artifact action, counter-pile targets, once-per-battle state, AI hint, and UI affordance. |
