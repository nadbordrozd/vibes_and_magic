...
The castle screen presents the visiting hero and current garrison as two compact, opposing
seven-slot rows. Their identity, occupied capacity, and unit counts are visible together. This
...
- No component mutates an army. The existing reducer remains solely responsible for count and
  identity conservation, whole-company swaps, seven-slot bounds, and installing a qualifying
...
the same transfer section, preserving both seven-slot rows without horizontal overflow. Slot
...
- `npm run review:ux` and `npm run review:castle-transfer` exercise direct full and partial actions,
  ...
  Escape cancellation,
  desktop/narrow seven-slot fit, illegal-reason feedback,
